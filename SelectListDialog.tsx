 ...
 * セレクトリストをもつdialog
 */
export default class SelectListDialog extends React.PureComponent<
...
> {
    ...
    public render() {
        ...
        const { open, searchWord, searchResults } = this.state
        return readonly ? (
    ...
    private handleListItemClick = (item: IItem) => {
        if (this.props.setFieldValue) {
            this.props.setFieldValue(this.props.field.name, item.id)
        }
        this.handleClose()
    }
     ...
     * @returns {string}
     */
    private getDisplayValue = (ids: number | number[]): string => {
        return this.props.items
            .filter((item: IItem) => item.id === ids)
